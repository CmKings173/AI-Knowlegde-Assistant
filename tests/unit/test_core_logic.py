...
@pytest.mark.asyncio
async def test_retriever_uses_configured_reranker() -> None:
    chunks = [
        _section_chunk("Backup email bang Outlook.", ["Email"], 1),
        _section_chunk("Mo NAS bang Windows R.", ["NAS"], 2),
    ]
    ...
    class FakeReranker:
        async def rerank(self, query, candidate_chunks, top_k):
            del query, top_k
            ranked = sorted(
                candidate_chunks,
                key=lambda chunk: "NAS" in chunk.content,
                reverse=True,
            )
            for index, chunk in enumerate(ranked):
                chunk.score = 1.0 - index * 0.1
            return ranked
    ...
    result = await retriever.retrieve("NAS")

    assert result.reranker_used
    assert result.chunks[0].content == "Mo NAS bang Windows R."
...
def test_fact_guard_rejects_procedural_claim_not_supported_by_context() -> None:
    result = validate_fact_consistency(
        (
            "\u0110\u1ec3 xin ngh\u1ec9 vi\u1ec7c, b\u1ea1n c\u1ea7n "
            "\u0111\u01b0\u1ee3c ph\u00e9p c\u1ee7a ng\u01b0\u1eddi qu\u1ea3n l\u00fd "
            "tr\u1ef1c ti\u1ebfp v\u00e0 ph\u1ea3i n\u00f3i r\u00f5 l\u00fd do. [SOURCE_1]"
        ),
        (
    ...
    assert result.reason == (
        "unsupported_support_term:PROCEDURE_MANAGER_APPROVAL,PROCEDURE_REASON"
    )


def test_fact_guard_allows_procedural_claim_supported_by_context() -> None:
    result = validate_fact_consistency(
        (
            "Khi xin ngh\u1ec9, b\u1ea1n ph\u1ea3i xin ph\u00e9p "
            "ng\u01b0\u1eddi qu\u1ea3n l\u00fd tr\u1ef1c ti\u1ebfp v\u00e0 n\u00f3i r\u00f5 "
            "l\u00fd do. [SOURCE_1]"
        ),
        (
            "Nh\u00e2n vi\u00ean xin ngh\u1ec9 ph\u1ea3i \u0111\u01b0\u1ee3c "
            "ng\u01b0\u1eddi qu\u1ea3n l\u00fd tr\u1ef1c ti\u1ebfp cho ph\u00e9p "
            "v\u00e0 ph\u1ea3i n\u00f3i r\u00f5 l\u00fd do xin ngh\u1ec9."
        ),
    )
...
def test_relevance_gate_rejects_context_without_important_query_terms() -> None:
    decision = validate_context_relevance(
        "quy tr\u00ecnh ph\u00ea duy\u1ec7t ngh\u1ec9 vi\u1ec7c",
        [
            _section_chunk(
                "Th\u1eddi gian l\u00e0m vi\u1ec7c t\u1eeb 8h00 \u0111\u1ebfn "
                "17h30, ngh\u1ec9 tr\u01b0a t\u1eeb 12h00 \u0111\u1ebfn 13h30.",
    ...
    assert decision.reason == "insufficient_query_term_coverage"


def test_relevance_gate_allows_context_with_important_query_terms() -> None:
    decision = validate_context_relevance(
        "quy tr\u00ecnh ph\u00ea duy\u1ec7t ngh\u1ec9 vi\u1ec7c",
        [
            _section_chunk(
                "Nh\u00e2n vi\u00ean ngh\u1ec9 vi\u1ec7c ph\u1ea3i th\u00f4ng "
                "b\u00e1o v\u00e0 \u0111\u01b0\u1ee3c ph\u00ea duy\u1ec7t tr\u01b0\u1edbc "
                "khi b\u00e0n giao.",
                ["Phan I: Noi quy", "Dieu 4"],
                4,
            )
        ],
    )
...
def test_parse_router_output_accepts_semantic_router_aliases() -> None:
    internal = parse_router_output(
        '{"intent": "internal_knowledge", "confidence": 0.82}'
    )
    source_challenge = parse_router_output(
        '{"intent": "source_challenge", "subtype": "source_challenge", "confidence": 0.8}'
    )

    assert internal is not None
    assert internal.intent == Intent.KNOWLEDGE_QUERY
    assert source_challenge is not None
    assert source_challenge.intent == Intent.FOLLOW_UP
    assert source_challenge.subtype == FollowUpSubtype.SOURCE_CHALLENGE
...
@pytest.mark.asyncio
async def test_pipeline_uses_semantic_router_for_uncertain_internal_hr_query(
    tmp_path: Path,
) -> None:
    class FakeRetriever:
        def __init__(self) -> None:
        ...
        async def retrieve(self, query, filters=None):
            ...
            return RetrievalResult(
                chunks=[
                    _section_chunk(
                        "Xin nghi phai duoc phep cua nguoi quan ly truc tiep "
                        "va phai noi ro ly do xin nghi.",
                        ["Phan I: Noi quy", "Dieu 1"],
                        1,
                    )
                ],
    ...
    class FakeLLM:
        def __init__(self) -> None:
            ...
            self.outputs = [
                (
                    '{"intent": "knowledge_query", "subtype": "none", '
                    '"confidence": 0.88, "reason": "semantic_hr_policy"}'
                ),
                (
                    '{"status": "answered", '
                    '"answer": "Xin nghỉ phải được phép của người quản lý trực tiếp '
                    'và phải nói rõ lý do xin nghỉ. [SOURCE_1]", '
                    '"sources": ["SOURCE_1"]}'
                ),
            ]
    ...
    result = await pipeline.answer("toi muon xin nghi han thi lam nhu nao")

    assert result["status"] == "answered"
    assert retriever.queries == ["toi muon xin nghi han thi lam nhu nao"]
    assert result["trace"]["llm_router_used"] is True
    assert result["trace"]["intent"] == "knowledge_query"
    assert result["citations"][0]["citation_id"] == "SOURCE_1"


@pytest.mark.asyncio
async def test_pipeline_rejects_hallucinated_procedural_claim_from_weak_context(
    tmp_path: Path,
) -> None:
    class FakeRetriever:
        async def retrieve(self, query, filters=None):
            return RetrievalResult(
                chunks=[
                    _section_chunk(
                        "Quy \u0111\u1ecbnh ngh\u1ec9 l\u00e0m trong \u0111i\u1ec1u n\u00e0y "
                        "ch\u1ec9 n\u00eau th\u1eddi gian ngh\u1ec9 tr\u01b0a. "
                        "Th\u1eddi gian l\u00e0m vi\u1ec7c t\u1eeb 8h00 s\u00e1ng "
                        "\u0111\u1ebfn 17h30 chi\u1ec1u t\u1eeb th\u1ee9 2 \u0111\u1ebfn "
                        "th\u1ee9 6, ngh\u1ec9 tr\u01b0a t\u1eeb 12h00 \u0111\u1ebfn "
                        "13h30.",
                        ["Phan I: Noi quy", "Dieu 1: Thoi gian lam viec"],
                        1,
                    )
                ],
                candidate_count=1,
    ...
    class FakeLLM:
        async def generate(self, system_prompt: str, user_prompt: str) -> str:
            return (
                '{"status": "answered", '
                '"answer": "\u0110\u1ec3 xin ngh\u1ec9 vi\u1ec7c, b\u1ea1n '
                'c\u1ea7n \u0111\u01b0\u1ee3c ph\u00e9p c\u1ee7a '
                'ng\u01b0\u1eddi qu\u1ea3n l\u00fd tr\u1ef1c ti\u1ebfp '
                'v\u00e0 ph\u1ea3i n\u00f3i r\u00f5 l\u00fd do. [SOURCE_1]", '
                '"sources": ["SOURCE_1"]}'
            )
    ...
    result = await pipeline.answer("quy \u0111\u1ecbnh ngh\u1ec9 l\u00e0m")

    assert result["status"] == "generation_failed"
    assert result["trace"]["fact_guard_error"] == (
        "unsupported_support_term:PROCEDURE_MANAGER_APPROVAL,PROCEDURE_REASON"
    )


@pytest.mark.asyncio
async def test_pipeline_refuses_before_llm_when_relevance_gate_fails(
    tmp_path: Path,
) -> None:
    class FakeRetriever:
        async def retrieve(self, query, filters=None):
            return RetrievalResult(
                chunks=[
                    _section_chunk(
                        "Th\u1eddi gian l\u00e0m vi\u1ec7c t\u1eeb 8h00 "
                        "\u0111\u1ebfn 17h30, ngh\u1ec9 tr\u01b0a t\u1eeb "
                        "12h00 \u0111\u1ebfn 13h30.",
                        ["Phan I: Noi quy", "Dieu 1"],
                        1,
                    )
    ...
    class FailLLM:
        async def generate(self, system_prompt: str, user_prompt: str) -> str:
            raise AssertionError("weak evidence should be refused before LLM generation")
    ...
    result = await pipeline.answer("quy tr\u00ecnh ph\u00ea duy\u1ec7t ngh\u1ec9 vi\u1ec7c")

    assert result["status"] == "insufficient_context"
    assert result["trace"]["branch"] == "knowledge_relevance_refusal"
    assert result["trace"]["relevance_error"] == "insufficient_query_term_coverage"


@pytest.mark.asyncio
async def test_pipeline_uses_semantic_router_for_low_confidence_hr_follow_up(
    tmp_path: Path,
) -> None:
    class FakeRetriever:
        def __init__(self) -> None:
        ...
        async def retrieve(self, query, filters=None):
            ...
            return RetrievalResult(
                chunks=[
                    _section_chunk(
                        "Phai co trach nhiem ban giao lai toan bo tai san "
                        "duoc giao khi nghi viec tai Cong ty.",
                        ["Phan I: Noi quy", "Dieu 4"],
                        4,
                    )
                ],
    ...
    class FakeLLM:
        def __init__(self) -> None:
            ...
            self.outputs = [
                (
                    '{"intent": "knowledge_query", "subtype": "none", '
                    '"confidence": 0.9, "reason": "semantic_hr_follow_up"}'
                ),
                (
                    '{"status": "answered", '
                    '"answer": "Khi nghỉ việc, nhân viên phải bàn giao lại toàn bộ '
                    'tài sản được giao. [SOURCE_1]", '
                    '"sources": ["SOURCE_1"]}'
                ),
            ]
    ...
    result = await pipeline.answer(
        "toi muon xin nghi lam han ay",
        history=[
            {"role": "user", "content": "toi muon xin nghi han thi lam nhu nao"},
            {
                "role": "assistant",
                "content": "Ban dang gap van de voi phan nao: NAS, Outlook...",
            },
        ],
    )

    assert result["status"] == "answered"
    assert retriever.queries == ["toi muon xin nghi lam han ay"]
    assert result["trace"]["llm_router_used"] is True
    assert result["trace"]["intent"] == "knowledge_query"


@pytest.mark.asyncio
async def test_pipeline_clarifies_low_confidence_follow_up_when_semantic_router_fails(
    tmp_path: Path,
) -> None:
    class FailRetriever:
        async def retrieve(self, query, filters=None):
            raise AssertionError("semantic router failure should not retrieve")
    ...
    assert result["status"] == "clarify"
    assert result["trace"]["llm_router_used"] is True
    assert result["trace"]["reason"] == "semantic_router_failed"
    assert result["retrieval"]["candidate_count"] == 0


@pytest.mark.asyncio
async def test_pipeline_stream_delegates_low_confidence_hr_follow_up_to_rag(
    tmp_path: Path,
) -> None:
    class FakeRetriever:
        async def retrieve(self, query, filters=None):
    ...
    class FakeLLM:
        ...
        async def stream_generate(self, system_prompt: str, user_prompt: str):
            raise AssertionError("low-confidence HR follow-up must not stream conversation")
            yield ""
    ...
    assert events[-1]["data"]["status"] == "answered"
    assert events[-1]["data"]["trace"]["llm_router_used"] is True
